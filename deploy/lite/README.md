...
| `TRANSCRIPTION_LANGUAGE` | — | Spoken language (ISO 639-1, e.g. `ru`) for bots whose `POST /bots` names none; an explicit `language` wins. Unset → the STT auto-detects per transcription window |
