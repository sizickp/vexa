...
<Tabs>
  ...
  <Tab title="Yandex Telemost">
    Telemost is **join + capture unit-tested; live validation pending**. The bot joins as a guest, so
    with the waiting room on (the default for Yandex 360 organisations) the organiser admits it.
