...
<Tabs>
  ...
  <Tab title="Yandex Telemost">
    Telemost: **join, speaker names and audio capture witnessed live; an end-to-end transcript run is
    pending**. The bot joins as a guest, so with the waiting room on (the default for Yandex 360
    organisations) the organiser admits it.

    ```bash
    # the numeric id from the /j/<id> link; send meeting_url too — it names the host
    # (telemost.yandex.ru, or telemost.360.yandex.ru for Yandex 360)
    curl -X POST "$API_BASE/bots" \
      -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" \
      -d '{"platform":"telemost","native_meeting_id":"12345678901234","meeting_url":"https://telemost.yandex.ru/j/12345678901234","bot_name":"Vexa"}'
    ```
  </Tab>
</Tabs>
...
A self-hosted deployment can make *forced* the default: `TRANSCRIPTION_LANGUAGE=ru` pins every bot
whose request omits `language`, and an explicit `language` on the request still wins (see
