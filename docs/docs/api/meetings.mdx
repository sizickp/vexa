...
† **Telemost is join + capture unit-tested; live validation is pending.** The bot joins as a guest
through the web client, so a meeting with the waiting room on (the default for Yandex 360
organisations) needs the organiser to admit it.
