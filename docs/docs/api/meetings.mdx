...
† **Telemost: join, speaker names — in the grid and with a screen shared — and end-to-end transcripts
are witnessed live on a compose stack.** The bot joins as a guest through the web client, so a meeting
with the waiting room on (the default for Yandex 360 organisations) needs the organiser to admit it.
...
For `telemost`, pass the full `meeting_url` (`https://telemost.yandex.ru/j/<id>`, or the
`telemost.360.yandex.ru` host of Yandex 360); the `native_meeting_id` is the numeric id from the
link. A meeting that requires a Yandex sign-in, or a link to a meeting that does not exist, ends the
join as `rejected` rather than being retried.
