...
| `TRANSCRIPTION_LANGUAGE` | — (auto-detect) | Spoken language (ISO 639-1, e.g. `ru`) for every bot whose `POST /bots` names none. Resolution: **explicit request `language` wins** → else this env → else the STT auto-detects per transcription window. Set it when your meetings share one language: auto-detect on short windows ("Thank you.", "Merci.") labels them as other languages. Also applies when the STT backend is configured in Settings — it names the speech, not the backend. |
